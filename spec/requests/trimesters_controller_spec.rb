...
RSpec.describe "Trimesters", type: :request do
  ...
  it 'returns a page with no trimesters' do
    get '/trimesters'
    expect(response.body).to include('<ul class="trimester-list">') 
    expect(response.body).to match(/<ul class="trimester-list">\s*<\/ul>/)  
      end
    end
  end
end